    ...
    with open(image+'.key', 'r') as f:
        ...
        for row in reader:
            ...
            if len(row) == 8:
                descriptor += row
                assert len(
                    descriptor) == 128, "Keypoint descriptor length invalid (should be 128)."
                # normalize the key to unit length
                descriptor = np.array(descriptor)
                descriptor = descriptor / \
                    math.sqrt(np.sum(np.power(descriptor, 2)))
...
def DisplayMatches(im1, im2, matched_pairs):
    ...
    for match in matched_pairs:
        draw.line((match[0][1], match[0][0], offset+match[1]
                  [1], match[1][0]), fill="red", width=2)
    im3.show()
    return im3
...
def FindBestMatchesXY(im_src_path, im_ref_path, ratio_thres):
    ...
    matches = list(sol.FindBestMatches(
        descriptors1, descriptors2, ratio_thres))
...
def VisualizePointProj(xy_src, xy_ref, xy_proj, im_src, im_ref):
    ...
    axes[1].scatter(xy_proj[:, 0], xy_proj[:, 1],
                    c='#fafba4', s=100, marker='.')
